...
- `Model.js` is the pure-JS core and single source of truth: iCalendar parsing, RRULE/DTSTART recurrence expansion, VTIMEZONE/DST handling, display labels, domain classes (`CalendarEvent`, `TimezoneResolver`, `RecurrenceRule`, `RecurrenceExpander`, `MeetingLinkDetector`, `IcsParser`, `JsonStateParser`, `FeedConfigParser`, `ScheduleAggregator`, `DisplayFormatter`, `PanelNavigationModel`, `LauncherResolver`, `LauncherSettings`), and navigation. No Qt/Quickshell imports.
